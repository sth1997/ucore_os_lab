...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2015011305 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    if (blkoff != 0) {	// (1)
    	...
    	if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) { //find the NO. of disk block: ino
    		goto out;	//out means 'finished'
    	}
    	if ((ret = sfs_buf_op(sfs, buf, size, ino, blkoff)) != 0) {	//write buf or read buf
    		goto out;
    	}
    	alen += size;
    	if (nblks == 0) {
    ...
    while (nblks != 0) { // (2)
    	...
    	if ((ret = sfs_block_op(sfs, buf, ino, 1)) != 0) {	//write buf or read buf (one block each time)
			goto out;
		}
    	alen += size;
    	buf += size;
    ...
    if ((size = endpos % SFS_BLKSIZE) != 0) { // (3)
    	if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {	//find the NO. of disk block: ino
			goto out;
		}
		if ((ret = sfs_buf_op(sfs, buf, size, ino, 0)) != 0) {	//write buf or read buf (one block each time)
			goto out;
		}
		alen += size;
    }
