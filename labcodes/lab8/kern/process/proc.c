...
alloc_proc(void) {
     ...
     */
    //LAB5 2015011305 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2015011305 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2015011305 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2015011305
    //LAB8:EXERCISE2 2015011305  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    */
    ...
    local_intr_save(intr_flag);
    {
        ...
        //add for LAB5
        set_links(proc);  //本函数包含list_add与nr_process++
        //list_add(&proc_list, &(proc->list_link));
        //nr_process++;
    }
    local_intr_restore(intr_flag);
...
load_icode(int fd, int argc, char **kargv) {
	...
	//(1) create a new mm for current process (mm_create)
	struct mm_struct *mm;
	if ((mm = mm_create()) == NULL) {
		goto bad_mm;
	}
	//(2) create a new PDT, and mm->pgdir= kernel virtual addr of PDT (setup_pgdir)
	if (setup_pgdir(mm) != 0) {
		goto bad_pgdir_cleanup_mm;
	}
	//(3) copy TEXT/DATA/BSS parts in binary to memory space of process
	struct Page *page;
	//(3.1) read raw data content in file and resolve elfhdr (load_icode_read)
	...
	//(3.2) read raw data content in file and resolve proghdr based on info in elfhdr (load_icode_read)
